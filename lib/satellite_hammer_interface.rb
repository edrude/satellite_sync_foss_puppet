...
class SatelliteHammerInterface
  def initialize(hammer_path: nil)
    ...
    @hammer_command = hammer_path
  end

  def run_satellite_hammer(args)
    args = ['--output', 'yaml'] + args
  ...
  def puppetenvironment_create(name, location_id: nil, organization_id: nil)
    args = %w[puppet-environment create]
    args << '--name' << name
    args << '--location-ids' << location_id.to_s if location_id
    args << '--organization-ids' << organization_id.to_s if organization_id
  ...
  def puppetenvironment_list(location_id: nil, organization_id: nil)
    ...
    args << '--location-id' << location_id.to_s if location_id
    args << '--organization-id' << organization_id.to_s if organization_id

    stdout, stderr, status = run_satellite_hammer(args)
    raise "Failed to list puppet environments\n#{stderr}" unless status.success?
